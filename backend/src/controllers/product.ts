 ...
 *    - Так как HTTP запрос не содержит параметров для фильтрации, ответ ошибки всегда будет со статусом 500 (Internal Server Error)
 * 
 * Используемая модель:
 ...
 * 
 * 2. В случае ошибки:
 *    - Если ошибка связана с конфликтом данных (дублирование уникального/уникальных поля/полей), создается ошибка типа ConflictError с пояснением проблемы.
 *    - Если ошибка вызвана неправильным запросом пользователя (некорректные данные), создается ошибка типа BadRequestError с пояснением проблемы.
 * 
 * Используемая модель:
 * - productModel для взаимодействия с базой данных.
 * 
